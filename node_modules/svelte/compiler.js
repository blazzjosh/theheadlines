(function (global, factory) {
	    ...
	    render(block, parent_node, parent_nodes) {
	        ...
	        if (!this.node.namespace && (this.can_use_innerhtml || this.can_use_textcontent()) && this.fragment.nodes.length > 0) {
	            if (this.fragment.nodes.length === 1 && this.fragment.nodes[0].node.type === 'Text') {
	                ...
	                to_html(this.fragment.nodes, block, literal, state);
	...
	function to_html(wrappers, block, literal, state) {
	...
	function analyze$1(expression) {
		const map = new WeakMap();

		let scope = new Scope$2(null, false);
		...
		walk(expression, {
			enter(node, parent) {
				if (node.type === 'ImportDeclaration') {
					...
					if (node.type === 'FunctionDeclaration') {
						scope.declarations.set(node.id.name, node);
						map.set(node, scope = new Scope$2(scope, false));
					} else {
						map.set(node, scope = new Scope$2(scope, false));
					...
					node.params.forEach((param) => {
						extract_names$1(param).forEach(name => {
							scope.declarations.set(name, node);
						});
					});
				} else if (/For(?:In|Of)?Statement/.test(node.type)) {
					map.set(node, scope = new Scope$2(scope, true));
				} else if (node.type === 'BlockStatement') {
					map.set(node, scope = new Scope$2(scope, true));
				} else if (/(Class|Variable)Declaration/.test(node.type)) {
					scope.add_declaration(node);
					...
					if (node.param) {
						extract_names$1(node.param).forEach(name => {
							scope.declarations.set(name, node.param);
						});
					}
				}
		...
		walk(expression, {
			enter(node, parent) {
				...
				if (node.type === 'Identifier' && isReference(node, parent)) {
					if (add_reference$1(scope, node.name) === root_scope) {
	...
	function add_reference$1(scope, name) {
		scope.references.add(name);
		if (scope.parent && !scope.declarations.has(name)) return add_reference$1(scope.parent, name);
		return scope;
	}
	...
	class Scope$2 {
		...
		constructor(parent, block) {Scope$2.prototype.__init.call(this);Scope$2.prototype.__init2.call(this);Scope$2.prototype.__init3.call(this);
			this.parent = parent;
			this.block = block;
		}
		...
		add_declaration(node) {
			if (node.type === 'VariableDeclaration') {
				if (node.kind === 'var' && this.block && this.parent) {
					...
					node.declarations.forEach((declarator) => {
						extract_names$1(declarator.id).forEach(name => {
	...
	function extract_names$1(param) {
		return extract_identifiers$1(param).map(node => node.name);
	}

	function extract_identifiers$1(param) {
		const nodes = [];
		extractors$1[param.type] && extractors$1[param.type](nodes, param);
		return nodes;
	}
	...
	const extractors$1 = {
		...
		ObjectPattern(nodes, param) {
			param.properties.forEach((prop) => {
				if (prop.type === 'RestElement') {
					...
					extractors$1[prop.value.type](nodes, prop.value);
				}
			});
		},
		...
		ArrayPattern(nodes, param) {
			param.elements.forEach((element) => {
				if (element) extractors$1[element.type](nodes, element);
			});
		},

		RestElement(nodes, param) {
			extractors$1[param.argument.type](nodes, param.argument);
		},

		AssignmentPattern(nodes, param) {
			extractors$1[param.left.type](nodes, param.left);
		}
	};

	function create_scopes(expression) {
	    return analyze$1(expression);
	}
	        ...
	        walk(component.ast.instance.content, {
	            ...
	            leave(node) {
	                ...
	                if (node.type === 'AssignmentExpression' || node.type === 'UpdateExpression') {
	                    ...
	                    const names = new Set(extract_names$1(assignee));
	...
	function ssr(component, options) {
	    ...
	    const reactive_declarations = component.reactive_declarations.map(d => {
	        ...
	        if (d.declaration) {
	            const declared = extract_names$1(d.declaration);
	...
	class Component {
	    ...
	    generate(result) {
	        ...
	        if (result) {
	            ...
	            const banner = `${this.file ? `${this.file} ` : ``}generated by Svelte v${'3.13.0'}`;
	    ...
	    walk_instance_js_pre_template() {
	        ...
	        script.content.body.forEach(node => {
	            ...
	            if (expression.type !== 'AssignmentExpression')
	                return;
	            extract_names$1(expression.left).forEach(name => {
	    ...
	    track_references_and_mutations() {
	        ...
	        walk(content, {
	            enter(node, parent) {
	                ...
	                if (node.type === 'AssignmentExpression' || node.type === 'UpdateExpression') {
	                    const assignee = node.type === 'AssignmentExpression' ? node.left : node.argument;
	                    const names = extract_names$1(assignee);
	                ...
	                walk(node.body, {
	                    enter(node, parent) {
	                        ...
	                        if (node.type === 'AssignmentExpression') {
	                            ...
	                            extract_identifiers$1(left).forEach(node => {
